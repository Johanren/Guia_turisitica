...
class ModeloUsuario{
    ...
    {
        $sql = "SELECT * FROM $this->tabla INNER JOIN rol ON usuario.id_rol = rol.id_rol INNER JOIN activo ON usuario.id_activo = activo.id_activo WHERE usuario.usuario = ? AND usuario.clave = ?";
        $conn = new Conexion();
    ...
    {
        if ($dato != null) {
            $sql = "SELECT * FROM $this->tabla INNER JOIN activo ON usuario.id_activo = activo.id_activo INNER JOIN rol ON usuario.id_rol = rol.id_rol WHERE usuario like ? OR rol.rol like ?";
        } else {
            $sql = "SELECT * FROM $this->tabla INNER JOIN activo ON usuario.id_activo = activo.id_activo INNER JOIN rol ON usuario.id_rol = rol.id_rol LIMIT ?,?";
        }
        try {
            $conn = new Conexion();
            $stms = $conn->conectar()->prepare($sql);
    ...
    {
        $sql = "INSERT INTO $this->tabla (usuario, clave, id_activo, id_rol) VALUES (?,?,?,?)";
        $conn = new Conexion();
        $stms = $conn->conectar()->prepare($sql);
        if ($dato != '') {
            $stms->bindParam(1, $dato['usuario'], PDO::PARAM_STR);
    ...
    {
        $sql = "UPDATE $this->tabla SET usuario=?,clave=?,id_activo=?,id_rol=? WHERE id_usuario=?";
        $conn = new Conexion();
        $stms = $conn->conectar()->prepare($sql);
        if ($dato != '') {
            $stms->bindParam(1, $dato['usuario'], PDO::PARAM_STR);
